...
emergencyRouter.route('/')
  .post(
    authenticateToken,
    body('res_id').isInt().withMessage('res_id must be an integer'),
    body('user_id').isInt().withMessage('user_id must be an integer'),
    body('pat_check').isBoolean().withMessage('pat_check must be true or false'),
    body('date').isNumeric().withMessage('date must be a timestamp'),
