...
async function getAlarms(docID) {
    try {
        const url = `http://35.228.145.163:3000/api/emergency/${docID}`; 
        const options = {
            method: 'GET',
            headers: {
...
async function getAlarmsMsg(docID) {
        try {
            const url = `http://35.228.145.163:3000/api/emergency/msg/${docID}`; 
            const options = {
                method: 'GET',
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`,
...
async function checkDocNotification([resID, tableID]) {
    ...
    try {
        const url = `http://35.228.145.163:3000/api/emergency/${urlEnd}${resID}`;
        const options = {
            method: 'PATCH',
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`,
...
const savePatientAlarm = async (oireet, userText) => {
    ...
    try {
        const url = `http://35.228.145.163:3000/api/emergency/${userID}`;
        const options = {
            body: JSON.stringify({
                symptoms: oireet,
                pat_msg: userText
