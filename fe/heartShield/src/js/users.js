...
const getUsers = async () => {
    ...
    let usersFill = false;
    if (userType === 'adm') {
        url = 'http://35.228.145.163:3000/api/users';
        blockTarget = '#list-of-users';
        usersFill = true;
    } else if (userType === 'doc') {
        url = `http://35.228.145.163:3000/api/users/${localStorage.getItem('user_id')}`;
...
const getPatReccomendations = async (patID) => {
    const url = `http://35.228.145.163:3000/api/users/recom/${patID}`;
    const options = {
        method: 'GET',
        headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
...
const getUserInfo = async (userId, userType) => {
    ...
    const url = `http://35.228.145.163:3000/api/users/${userType}/${userId}`;
    const options = {
        method: 'GET',
        headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
...
const getLastAiResponse = async (userId) => {
    const url = `http://35.228.145.163:3000/api/ai/${userId}`;
    const options = {
        method: 'GET',
        headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
...
const getFullAiResponse = async (userId) => {
    const url = `http://35.228.145.163:3000/api/ai/full/${userId}`;
    const options = {
        method: 'GET',
        headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
...
const deleteUserById = async (userId) => {
    try {
        const url = `http://35.228.145.163:3000/api/users/${parseInt(userId)}`;
        const options = {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`,
...
const searchAdmUser = async () => {
    ...
    if (!userID || userID.error) {
        ...
        const url = `http://35.228.145.163:3000/api/users/admin/${userID}`;
        const options = {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`
...
const loadAddData = async (patientData) => {
    ...
    const url = `http://35.228.145.163:3000/api/users/${id}`;
    const options = {
        body: JSON.stringify({
            age: age,
            mi_date: mi,
...
const loadNewUser = async () => {
    ...
    try {
        const url = `http://35.228.145.163:3000/api/users/`;
        const options = {
            body: JSON.stringify({
                name: user.name,
...
const getPatAddInfo = async (patID) => {
    const url = `http://35.228.145.163:3000/api/users/info/${patID}`;
    const options = {
        method: 'GET',
        headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
