...
**POST	/api/login** - Авторизация пользователя (email + пароль)
...
**GET     /api/users/:id** - Получение списка пациентов (врач)
**DELETE  /api/users/:id** - удаление пользователя
**GET     /api/admin/:id** - Получение основной информации пользователя
**GET     /api/info/:id** - Получение полной информации пациента (врач)
**GET     /api/:status/:id** - Получение регистрационной информации пользователя

### **📊 Измерения и анализ**

Метод	Маршрут	Описание

**POST    /api/users/:id** - Сохранение доп информации пациента
**GET     /api/recom/:id** - Получение списка рекомендаций для пациента
**POST    /api/recom/:id** - Создание рекомендации для пациента
**GET     /api/metrics/:id** - Получение метрик пациента из БД
**POST    /api/metrics/:id** - Сохранение метрик пациента
**POST    /api/emergency** - Сохранить новую Аларму
**GET     /api/emergency/:id** - Проверка Алармов (врач)
**POST    /api/emergency/:id** - Создание Алармы пациентом
**PATCH   /api/emergency/:id** - Отключение Алармы (просмотрено)
**GET     /api/emergency/msg/:id** - Получение списка Алармов пациента
**PATCH   /api/emergency/msg/:id** - Отключение Алармы пациента

### **🤖 Связь с ИИ **

Метод	Маршрут	Описание

**POST    /api/ai** - Отправка данных пациента на проверку ИИ

**GET     /api/ai/:id** - Получить последний ответ ИИ

**POST    /api/ai/:id** - Сохранить ответ ИИ

**GET     /api/ai/full/:id** - Получить все ответы ИИ

### 9.2 Структура данных

**Пример отправки измерений:**
...
**Пациент:** доступ только к собственным данным

**Врач:** доступ к расширенной информации о своих пациентах

**Админ:** доступ к регистрационным данным пользователей

Регистрация возможна только через уже авторизованных врачей (пациенты) или администраторов (врачи и админы)

## 10. Благодарности
Проект HeartShield был реализован в рамках курса Hyte-2025 под академическим руководством **преподавателей Metropolia AMK**.
